...
export class Renderer {
  ...
  async render(
    ...
    for (const managedFile of this.managedFiles(profile)) {
      const source = stringValue(managedFile.source, "managed source");
      const sourcePath = join(this.model.root, source);
      const targetPath = join(outputRoot, stringValue(managedFile.target, MANAGED_TARGET_LABEL));
      if (extname(targetPath) === MARKDOWN_EXTENSION) {
        const filtered = filterManagedText(await Bun.file(sourcePath).text(), selected, known, source);
        await writeBytes(targetPath, new TextEncoder().encode(filtered).buffer, normalizedMode(sourcePath));
      } else {
