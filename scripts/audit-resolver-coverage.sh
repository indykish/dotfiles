#!/usr/bin/env bash
# scripts/audit-resolver-coverage.sh — coherence audit for the resolver façade
...
#
#   (a) a [DETERMINISTIC → CODE] tag has no row for CODE in ANY resolver .sh.
...
# resolver-evals — not synced into product repos).
set -uo pipefail
...
check_tag_to_check() {
  ...
  while IFS= read -r code; do
    [ -z "$code" ] && continue
    if printf '%s\n' "$backed" | in_list "$code"; then okln "$code — wired in some resolver .sh"
    else fail "$code — DETERMINISTIC tag with no run/delegate row in any resolver .sh"; fi
  done < <(det_tags | real)
}
...
check_probe_coverage() {
  sec "(c) JUDGMENT tag → comprehension probe (fixtures.jsonl mode resolver-judgment-<code>)"
  local code lc
  while IFS= read -r code; do
    [ -z "$code" ] && continue
    lc="$(printf '%s' "$code" | tr '[:upper:]' '[:lower:]')"
    if grep -qF "\"mode\":\"resolver-judgment-$lc\"" "$PROBES"; then okln "$code — probe present"
    else fail "$code — JUDGMENT tag with no comprehension probe (expected mode resolver-judgment-$lc)"; fi
  done < <(jud_tags | real)
}

# ---- check (d) — orphan: every .sh code is tagged somewhere -----------------
check_orphan() {
  sec "(d) .sh code → tag in some resolver .md (orphan check, global)"
  local tags code; tags="$( { det_tags; jud_tags; } | sort -u )"
  while IFS= read -r code; do
    [ -z "$code" ] && continue
    if printf '%s\n' "$tags" | in_list "$code"; then okln "$code — tagged in some .md"
    else fail "$code — .sh dispatch row with no [.. → $code] tag in any resolver .md"; fi
  done < <(all_sh_codes)
}
...
check_leaf_presence() {
  ...
  while read -r code script; do
    [ -z "$code" ] && continue
    path="$SCR/$script"
    if [ -f "$path" ] && [ -x "$path" ]; then okln "$code → scripts/$script (present, executable)"
    else fail "$code → scripts/$script absent or non-executable — silent-green hole"; fi
...
check_gloss_presence() {
  sec "(f) .sh code → lib.sh RESOLVER_GLOSS entry (no naked codes)"
  local codes code; codes="$(gloss_lib | cut -f1)"
  while IFS= read -r code; do
    [ -z "$code" ] && continue
    if printf '%s\n' "$codes" | in_list "$code"; then okln "$code — gloss present"
    else fail "$code — emitted by a .sh row but has no RESOLVER_GLOSS entry"; fi
  done < <(all_sh_codes)
}

# ---- check (g) — RULES.md canonical legend == lib.sh RESOLVER_GLOSS ---------
...
printf '%sRESOLVER COVERAGE AUDIT — façade-pair coherence (6.3 + 6.4)%s\n' "$BO" "$X"
check_tag_to_check
check_fixture_coverage
check_probe_coverage
check_orphan
...
if [ "$RC" -eq 0 ]; then printf '\n%s✅ RESOLVER COVERAGE: ALL CHECKS PASSED%s\n' "$G$BO" "$X"
else printf '\n%s❌ RESOLVER COVERAGE: coherence gaps above — fix before commit%s\n' "$R$BO" "$X"; fi
exit "$RC"
