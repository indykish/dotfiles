...
- **Docs-repo edits land on their own branch — never on whatever branch happens to be checked out.** `~/Projects/docs/` (the public docs site backing `docs.usezombie.com` and `changelog.mdx`) is shared across milestones. The repo will often have an in-flight branch for the milestone the docs team is working on (M46-frontmatter, M47-approval-inbox, etc.). When the *current code milestone* needs a changelog or doc update, do NOT add it to the in-flight docs branch — that conflates two milestones into one PR. Instead: (1) check `cd ~/Projects/docs && git status` BEFORE editing; (2) if HEAD is on a non-`main` branch, either checkout `main` first OR `git worktree add` a fresh worktree off `main`; (3) make the edit on a milestone-specific branch (`chore/m{N}-{slug}-changelog`); (4) commit + push only that branch; (5) the user's PR-merge order owns when the changelog actually lands. If a docs-repo edit is already made on the wrong branch, the recovery is `git stash` (or `git diff > /tmp/doc.patch && git checkout .`), then re-apply on a fresh branch.
