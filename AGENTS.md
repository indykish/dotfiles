...
**Address tags.** Kishore: **🤠 Indy** (or plain `Indy` / `Kishore`); agent: **🦉 Orly** (or `Oracle`). Swap on request. **Project name:** rebrand `usezombie` → `agentsfleet` (spec M92_002; domains → `agentsfleet.net`). Write the product as `agentsfleet` (inline code), never bare; a `usezombie` brand representation → flag, replace when in scope, inform Indy. Product names use `agentsfleet` / `agentsfleetd` / `agentsfleet-runner`; entity/API names use `agent`, `agent_id`, `/agents`, `core.agent_*`. Keep until cutover: `usezombie.sh` (→ `agentsfleet.dev`), `@usezombie/*`, `~/Projects/usezombie`.
...
One per active stream. Stay inside; no edits outside, no reads from siblings. Merge only after VERIFY. `git checkout main && git branch feat/mNN-name && git worktree add ../agentsfleet-mNN-name feat/mNN-name && cd ../agentsfleet-mNN-name && bun install && (cd agentsfleet && bun install && bun run build)`. The root `bun install` hydrates the workspace (`ui/packages/*`); `agentsfleet/` is its own Bun project — install + build (mandatory because `test-unit-agentsfleet` spawns `dist/bin/agentsfleet.js` for `--help` / flag / validator assertions). Without these, `lint-app` / `lint-agentsfleet` / `test-unit-agentsfleet` fail on the first commit with `tsc: command not found`, missing `@assistant-ui/react`, or missing-binary symptoms. Post-merge: `git worktree remove ../agentsfleet-mNN-name`.
