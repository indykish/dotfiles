...
| `ui/packages/**/*.{tsx,jsx,css}` or any file changing visual tokens / motion / typography | `docs/DESIGN_SYSTEM.md` (when present) — design system source of truth: typography stack, color tokens, the single accent and its currency rule, motion signature, spacing/density, component principles, CLI palette mapping. DOC READ GATE per edit. |
