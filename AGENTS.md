...
**Address tags.** Address Kishore as **🤠 Indy** (or plain `Indy` / `Kishore`); the agent signs as **🦉 Orly** (or `Oracle`). One glyph each — swap on request. **Project name:** rebranding `usezombie` → `agentsfleet` (spec M92_002; domains → `agentsfleet.net`). Write the product as `agentsfleet` (inline code), never a bare word; a `usezombie` brand representation → flag, replace when in scope, inform Indy. Still-resolving operational names keep until their cutover spec: `usezombie.sh` (→ `agentsfleet.dev`), `github.com/usezombie`, `@usezombie/*`, `zombiectl`/`zombied`, `core.zombie_*`, `~/Projects/usezombie`.
