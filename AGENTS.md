...
**Address tags.** Kishore: **🤠 Indy** (or plain `Indy` / `Kishore`); agent: **🦉 Orly** (or `Oracle`). Swap on request. **Project name:** rebrand `usezombie` → `agentsfleet` (spec M92_002; domains → `agentsfleet.net`). Write the product as `agentsfleet` (inline code), never bare; a `usezombie` brand representation → flag, replace when in scope, inform Indy. Keep until cutover: `usezombie.sh` (→ `agentsfleet.dev`), `github.com/usezombie`, `@usezombie/*`, `zombiectl`→`agentsfleet`, `zombied`→`agentsfleetd`, `zombie-runner`→`agentsfleet-runner`, `core.zombie_*`, `~/Projects/usezombie`.
