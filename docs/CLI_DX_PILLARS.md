<!--
VENDORED into dotfiles from ~/Projects/oss/cli/docs/adr/0001-cli-dx-architecture-pillars.md
(the supabase-style CLI's "7 Pillars" Architecture Decision Record). Kept here so CLI-category
specs have a stable, in-repo prior-art reference — see docs/TEMPLATE.md "Prior-Art" and
kishore-spec-new Step 2. The oss/cli copy is canonical; re-sync if it changes. Cross-ADR links
below (e.g. 0007) resolve only in the source repo.
-->
