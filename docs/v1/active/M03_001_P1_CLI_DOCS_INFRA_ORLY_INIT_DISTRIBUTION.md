...
- **Dimension 7.3** — an existing `.githooks/pre-commit` orly did not write is refused, naming `--force`, never overwritten silently → Test `install > a hook orly did not write is refused, not clobbered`
