...
- **Dimension 7.1** — a repository carrying its own `AGENTS.md` installs without losing a byte of it; orly's rules land as `AGENTS.orly.md` → Test `install > an existing AGENTS.md survives and orly lands beside it`
- **Dimension 7.2** — the pointer orly adds to the repository's `AGENTS.md` sits in a marked block, is idempotent across re-runs, and is the only part of that file orly ever touches → Test `install > the orly pointer block is idempotent across repeated installs`
...
- **Dimension 7.4** — `AGENTS.orly.md` is regenerated by every `orly update`; the repository's own `AGENTS.md` is not → Test `install > update rewrites the orly file and leaves the repository file alone`
- **Dimension 7.5** — pack files whose resolved source equals their resolved target are skipped, so the engine checkout updates itself without overwriting its own sources → Test `install > a self-install skips same-path pack sources`
- **Dimension 7.6** — `orly sync` and `orly render` are deleted; `orly update` covers the engine checkout and `orly init --dry-run` covers the preview → Test `cli > sync and render are gone`
- **Dimension 7.7** — `orly init --with <pack>` records the pack in `.oracle/orly.json` and renders it, so an opt-in pack is one command → Test `cli > --with records and renders an opt-in pack`
- **Dimension 7.8** — the skills the rules name are materialised into the repository, so a rule never mandates a skill the repository lacks → Test `install > selected skills land in every supported agent directory`
- **Dimension 7.9** — `persona.indy` carries no vault name, no private repository inventory, and no work email, so enabling it is safe in a shared repository → Test `render > persona carries no vault or repository inventory`
