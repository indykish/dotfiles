...
- **Dimension 5.1** — DONE — the workflow runs the audit chain, fails on a seeded governance violation, and references no absolute developer path → Test `test_ci_gates_release_without_developer_paths`
...
### §7 — Installing costs the repository nothing it already had

Folded in after §6, on Kishore's direction, when the contributor walkthrough was run against a repository that already carried its own rules and its own hooks. Two paths lost user data: an existing `AGENTS.md` dead-ended the install with a refusal whose only documented escape was `--force`, which overwrites it; an existing `.githooks/pre-commit` was overwritten silently, reported as a normal write. Both are measured, not inferred — a fixture repository with a hand-written hook lost it and `init` printed `🟢 24 written`. The repository keeps its own file under its own name; orly's rules land beside it as `AGENTS.orly.md` and are reached by a pointer orly maintains. Nothing orly did not write is replaced without the user asking for it.

- **Dimension 7.1** — DONE — every install produces the same two files: `AGENTS.md` the repository's, `AGENTS.orly.md` orly's. A repository carrying its own keeps it byte for byte; one with none gets a stub it owns from the start → Test `install > an existing AGENTS.md survives byte for byte and orly lands beside it`, `install > a repository with no AGENTS.md still gets both files`
- **Dimension 7.10** — DONE — a single-file install from before the split migrates on the next update without losing a repository-authored line → Test `install > a single-file install migrates to the split without losing a repository line`
- **Dimension 7.11** — DONE — rule pages materialised into a consumer cite `AGENTS.orly.md`, the file orly wrote, never the repository's own `AGENTS.md` → Test `install > rule pages cite the file orly actually wrote, not the repository's`
- **Dimension 7.2** — DONE — the pointer orly adds to the repository's `AGENTS.md` sits in a marked block, is idempotent across re-runs, and is the only part of that file orly ever touches → Test `install > the orly pointer block is idempotent across repeated installs`
- **Dimension 7.3** — DONE — an existing `.githooks/pre-commit` orly did not write is refused, naming `--force`, never overwritten silently; and a hook that is a symlink out of the repository is refused even under `--force` → Test `install > a hook orly did not write is refused, not clobbered`, `install > --force still refuses a hook that is a symlink out of the repository`
- **Dimension 7.12** — DONE — every write orly makes is bounded by the repository, including the pointer block into a file it does not own; a symlinked destination file is refused, not followed → Test `install > refuses when the pointer host is a symlink escaping the target repository`
- **Dimension 7.13** — DONE — `--dry-run` writes nothing at all, including the opt-in pack a `--with` on the same command line would otherwise record → Test `cli > --dry-run records no pack`
- **Dimension 7.4** — DONE — `AGENTS.orly.md` is regenerated by every `orly update`; the repository's own `AGENTS.md` is not → Test `install > update rewrites the orly file and leaves the repository file alone`
- **Dimension 7.5** — DONE — pack files whose resolved source equals their resolved target are skipped, so the engine checkout updates itself without overwriting its own sources → Test `install > a self-install skips same-path pack sources`
- **Dimension 7.6** — DONE — `orly sync` and `orly render` are deleted; `orly update` covers the engine checkout and `orly init --dry-run` covers the preview → Test `cli > sync and render are gone`
- **Dimension 7.7** — DONE — `orly init --with <pack>` records the pack in `.oracle/orly.json` and renders it, so an opt-in pack is one command → Test `cli > --with records and renders an opt-in pack`
- **Dimension 7.8** — DONE — the skills the rules name are materialised into the repository, so a rule never mandates a skill the repository lacks → Test `install > selected skills land in every supported agent directory`
- **Dimension 7.9** — DONE — `persona.indy` carries no vault name, no private repository inventory, and no work email, so enabling it is safe in a shared repository → Test `render > persona carries no vault or repository inventory`
...
- **Metrics review** — no analytics/funnel playbook update required; unchanged from the spec's own row (no product/operator signal changes — internal developer tooling).
- **Skill-chain outcomes:**
  ...
  - `/review` — adversarial pass against the diff (base `bc0fe0a`) found four issues, all resolved:
    ...
    3. **LOW.** `.github/workflows/harness.yml` had no `permissions:` block, inheriting the write-scoped default token on a public repository. Fixed: added `permissions: contents: read` plus `--ignore-scripts` on the install step.
    4. **LOW, Indy's call.** `orly/repositories.json` — Indy's personal project registry (`agentsfleet`/`cache-kit.rs`/`docs`/`dotfiles` paths) — shipped in the public npm payload. Indy decided to remove it from publication. Fixed: excluded from `package.json`'s `files`, and `RulesModel.load()` now tolerates the file's absence (defaults to an empty repositories object) instead of throwing; Indy's own checkout keeps the real file tracked in git, `orly gate` unaffected. Regression coverage: `install_package_allowlist_excludes_personal_files` asserts the file never ships, `install_works_with_no_repositories_json_shipped` asserts `validate`/`init` both still work without it.

    Full suite re-run after all four fixes: `bun test src` 96/96, `bash evals/install/run.sh` 21/21, `make audit` ALL CHECKS PASSED, `actionlint` clean, `gitleaks detect` no leaks.
