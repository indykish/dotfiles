# Resolver Review — Finding Disposition

Date: Jun 04, 2026
Companion to `RESOLVER_ARCHITECTURE.md` (v2). Source: the 7-lens adversarial
...
| 3 | P0 | cross-repo | resolvers/write_zig.sh delegates to leaf scripts that cannot run where the resolver lives —  | partial | §0, §10, §10.3, §13, §6.3, §7 |
...
| 10 | P0 | ergonomics-robustness | resolver_run_helper returns 0 when the leaf script is absent — silent false-green in the rep | partial | §0, §10.2, §10.3, §12, §14, §6.1, §6.3 |
| 11 | P0 | eval-loop | §6.2 "extend existing scripts/llmevals/" is a category error — the existing harness is an ex | partial | §0, §13, §6.2, §6.3 |
...
| 13 | P0 | eval-loop | audit-agents-md.sh is hard-bound to docs/gates/ in four checks; dissolving the directory mak | resolved | §13, §8, §9 |
...
| 23 | P1 | cross-repo | The resolver absorbs the FILE cap but silently drops the function/method length sub-gate the | resolved | §0, §13, §5 |
...
| 30 | P1 | eval-loop | audit-resolver-coverage.sh asserts a scenario EXISTS, not that it is discriminating — one pa | partial | §3, §6.1, §6.3 |
...
| 37 | P2 | ergonomics-robustness | pre-commit latency: 5 resolvers each shell out to git diff + multiple full-tree audits on ev | partial | §7 |
| 38 | P2 | eval-loop | resolver_run_helper calls audit-ufs.sh --all on the WHOLE tree per edit, contradicting the " | resolved | §7 |
